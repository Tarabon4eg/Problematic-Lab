<?php
/**
 * Repository CustomerVisitedUrlsRepository
 *
 * @category  Smile
...
class CustomerVisitedUrlsRepository implements CustomerVisitedUrlsRepositoryInterface
{
    ...
    public function getByClientIp(string $clientIp)
    {
        ...
        $model = $this->modelFactory->create();
            ...
            $this->resourceModel->load($model, $clientIp, CustomerVisitedUrlsInterface::CLIENT_IP);
        if (!$model->getId()) {
            throw new NoSuchEntityException(__('No such client with IP: %1 ', $clientIp));
        }
